...
module.exports = function (deployer, network, accounts) {
  deployer.then(async () => {
    ...
    console.log('Created PriceFeedCommunity at', priceTx.receipt.logs[0].args.community);
    ...
    // Buy tokens
    const curve = await BondingCurve.at(await priceFeedCommunity.bondingCurve());
    ...
    await Promise.all(dataProviders.map(
      async dataSource => priceTCD.register(accounts[0], '500000000000000000000', dataSource)
    ));
  }).catch(console.log);
};
