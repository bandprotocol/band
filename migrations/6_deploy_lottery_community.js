...
module.exports = function (deployer, network, accounts) {
  deployer.then(async () => {
    ...
    console.log('Created LotteryFeedCommunity at', lotteryTx.receipt.logs[0].args.community);
    ...
    // Buy tokens
    const curve = await BondingCurve.at(await lotteryFeedCommunity.bondingCurve());

    await band.approve(curve.address, '620000000000000148973918');
    await curve.buy(accounts[0], '620000000000000148973918', '1000000000000000000000000');
    ...
    await Promise.all(dataProviders.map(
      async dataSource => lotteryTCD.register(accounts[0], '500000000000000000000', dataSource)
    ));
  }).catch(console.log);
};
