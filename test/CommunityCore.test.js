...
contract('CommunityCore', ([ _, owner, alice, bob, carol, deactivator ]) => {
  ...
  context('Checking buy and sell community tokens with f(s) = x ^ 2', () => {
    ...
    it('should increase price for subsequent purchases', async () => {
      ...
      it('should allow community token owner to sell with proper price drop',
         async () => {
           await this.band.approve(this.core.address, 100000, {from : alice});
           await this.core.buy(100, 11000, {from : alice});
           ...
           (await this.core.curveMultiplier())
               .should.bignumber.eq(1000000000000);
         });
    });
  });
  ...
  context('Checking sales commission', () => {
    beforeEach(async () => {
      ...
      await this.core.buy(100, 20000, {from : alice});
      // 20% sales tax
      await this.params.propose([ 'core:sales_commission' ], [ 200000000000 ], {
        from : alice,
      });
      await this.params.vote(1, 100, 0, {from : alice});
  ...
  context('Checking deflation feature', () => {
    ...
    it('should not allow non-admin to deflate',
       async () => { await reverting(this.core.deflate(10, {from : alice})); });

    it('should not allow admin to deflate more than what he or she owns',
       async () => {
    ...
    it('should allow admin to deflate', async () => {
      ...
      // 100 seconds have passed, and no one challenges, so now Alice is good to
      // go.
      await increase(100);
  ...
  context('Checking migration feature', () => {
    ...
    context('After the contract is deactivated', () => {
      ...
      it('should allow creating new community core with new equation',
         async () => {
           ...
           await this.comm.transferOwnership(core.address,
                                             {from : deactivator});
  ...
  context('Checking reward distribution feature', () => {
    ...
    context('After admin report reward', async () => {
      ...
      it('should not allow members to withdraw before edit period',
         async () => {
           await reverting(
      ...
      it('should not allow members to withdraw with invalid value',
         async () => {
           await increase(duration.days(1.5));
           await this.core.claimReward(1, 10, this.merkle.getProof(alice)[1], {
             from : alice,
      ...
      it('should allow extend edit period of admin overwrites the distribution hash',
         async () => {
      ...
      it('should allow another reward distribution after reward_period',
         async () => {
