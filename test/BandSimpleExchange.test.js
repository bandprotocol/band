...
contract('BandSimpleExchange', ([owner, alice]) => {
  beforeEach(async () => {
    ...
    this.bse = await BandSimpleExchange.new(
        this.band.address,
        { from: owner },
    );
  ...
  context('Exchange Basic Functionality', () => {
    ...
    it('Owner should be able to add BandToken to exchange', async () => {
      ...
      (await this.band.balanceOf(owner)).toNumber().should.eq(ownerPrevBalance - 50000);
    ...
    it('BandToken cant be added to exchange by none owner address', async () => {
      await shouldFail.reverting(
          this.bse.addBand(alice, 1, {from: alice})
      );
    });
    it('Owner can set exchange rate', async () => {
      (await this.bse.exchangeRate()).toString().should.eq('10000000000000000000');
      await this.bse.setExchangeRate('5000000000000000000',{from:owner});
      (await this.bse.exchangeRate()).toString().should.eq('5000000000000000000');
    });
    ...
    it('Can convert ETH to BandToken', async () => {
      ...
      (await this.band.balanceOf(this.bse.address)).toString().should.eq("10000000");

      await this.bse.convertFromEthToBand({ from: owner, value: 9000 });

      (await this.band.balanceOf(owner)).toString().should.eq("89000000");
      (await this.band.balanceOf(this.bse.address)).toString().should.eq("1000000");
    });
  });
});
