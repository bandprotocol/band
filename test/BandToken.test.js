...
contract('BandToken', ([_, owner, alice, bob]) => {
  ...
  it('should have correct information', async () => {
    ...
    (await this.contract.decimals()).should.bignumber.eq(18);
  });
  ...
  context('Without token locking', () => {
    it('should only allow valid token transfer', async () => {
      ...
      (await this.contract.balanceOf(alice)).should.bignumber.eq(900);
      (await this.contract.balanceOf(bob)).should.bignumber.eq(100);
    });
  ...
  context(
    ...
    () => {
      ...
      it('should only allow transfer after the cliff ends', async () => {
        ...
        (await this.contract.unlockedBalanceOf(alice)).should.bignumber.eq(250);
        // Alice sends 150 of the vested tokens to Bob.
        await this.contract.transfer(bob, 150, { from: alice });
        (await this.contract.unlockedBalanceOf(alice)).should.bignumber.eq(100);
        ...
        (await this.contract.unlockedBalanceOf(alice)).should.bignumber.eq(767);
        await increaseTo(1580515200); // 2020/02/01
        // All fully vested. 1000 - 150 = 850 should be available!
        (await this.contract.unlockedBalanceOf(alice)).should.bignumber.eq(850);
      });
    },
  );
});
