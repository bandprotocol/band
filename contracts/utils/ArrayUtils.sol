...
library ArrayUtils {
  ...
  /// Return the median value of the given array. Revert if array is empty.
  function getMedian(uint256[] memory data)  internal pure  returns (uint256 median) {
  ...
  /// Return the "majority" value in the given array. A value is considered majority iff
  /// more than half of the array elements are that value. Revert if none exists.
  function getMajority(uint256[] memory data) internal pure returns (uint256 majority) {
    for (uint256 i = 0; i < data.length; ++i) {
      uint256 count = 1;
