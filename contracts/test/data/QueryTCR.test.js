...
contract(
  'QueryTCR',
  ([_, owner, alice, bob, carol, minProposer, minChallenger]) => {
    beforeEach(async () => {
      ...
      const calldata100 = this.curve.contract.methods
        .buy(_, 0, 100)
        .encodeABI();
    ...
    context('Voting for an on going challenge', () => {
      const entryHash = web3.utils.soliditySha3('some entry');
      const reasonHash = web3.utils.soliditySha3('some reason');
      const entryHash2 = web3.utils.soliditySha3(
        'some entry with minimum stake',
      );
      const reasonHash2 = web3.utils.soliditySha3(
        'challenge with minimum stake',
      );
      const salt = 99;
      beforeEach(async () => {
        let calldata = await this.tcr.contract.methods
    ...
    context('Claim reward flow', () => {
      ...
      it('Should start with initial vote from proposer and challenger', async () => {
        ...
        (await this.tcr.getVoteStatus(challengeId, alice))
          .toNumber()
          .should.eq(2);
        (await this.tcr.getVoteStatus(challengeId, bob))
          .toNumber()
          .should.eq(0);
      ...
      it('Should distrubute reward correctly when remove entry', async () => {
        ...
        await expectRevert.unspecified(this.tcr.currentMinDeposit(entryHash));
        ...
        const bobReward = Math.floor(
          ((70 - challengerVoteReward) * 800) / 1800,
        );
        (await this.comm.balanceOf(bob)).toNumber().should.eq(800 + bobReward);
        (await this.comm.balanceOf(carol))
          .toNumber()
