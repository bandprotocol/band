...
export const getNBATeamByCode = code => {
  return NBA_TEAM[
    Object.keys(NBA_TEAM).filter(each => NBA_TEAM[each].value.includes(code))[0]
  ]
}

export const getEPLTeamByCode = code => {
  return EPL_TEAM[
    Object.keys(EPL_TEAM).filter(each => EPL_TEAM[each].value.includes(code))[0]
  ]
}

export const getMLBTeamByCode = code => {
  return MLB_TEAM[
    Object.keys(MLB_TEAM).filter(each => MLB_TEAM[each].value.includes(code))[0]
  ]
}

export const getNFLTeamByCode = code => {
  return NFL_TEAM[
    Object.keys(NFL_TEAM).filter(each => NFL_TEAM[each].value.includes(code))[0]
  ]
}
...
export const getOptionsByPrefix = type => {
  switch (type) {
    ...
    case 'nfl':
      return NFL_TEAM
    case 'tcd':
      return NBA_TEAM.concat(EPL_TEAM, MLB_TEAM, NFL_TEAM)
    default:
