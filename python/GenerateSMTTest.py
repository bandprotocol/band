...
# js code
print('const rootHash = "0x{}";'.format(m.root.to_bytes(32, 'big').hex()))
print('const key = "0x{}";'.format((A[0]).to_bytes(20, 'big').hex()))
print('const value = "0x{}";'.format((A[1]).to_bytes(32, 'big').hex()))
# print('uint mask = {};'.format(mask))
print('const proofs = [')
for i in range(len(proof)):
    print('"0x{}",'.format(proof[i].to_bytes(32, 'big').hex()))

newV = random.randint(0, 2 ** 256 - 1)
m.insert(A[0], newV)

print('const newValue = "0x{}";'.format((newV).to_bytes(32, 'big').hex()))

print('const newRootHash = "0x{}";'.format(m.root.to_bytes(32, 'big').hex()))
